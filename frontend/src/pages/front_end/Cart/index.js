...
const Cart = () => {
  ...
  const updateItemQuantity = async (quantity) => {
    setIsLoading(true)
    await dispatch(userActions.updateItemQuantity(quantity))
    setIsLoading(false)
  }

  const deleteCartItem = (itemInfo, priceNumber) => {
    setTotalCash((prev) => prev - priceNumber)
    dispatch(userActions.deleteCartItem(itemInfo))
    ...
    <>
      ...
      <Stack direction="horizontal" gap={5}>
        <div className="bg-light border ms-auto">
          <h3 className="fw-bold">Totall:{totalCash}$</h3>
        </div>
        <Button
